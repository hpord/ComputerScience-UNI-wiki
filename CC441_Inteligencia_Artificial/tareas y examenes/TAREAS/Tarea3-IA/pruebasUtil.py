...
class Part:
    def __init__(self, name, gradeFunc, maxPoints, maxSeconds, extraCredit, description, basic):
        if not isinstance(name, str):
            raise Exception("Nombre invalido: %s" % name)
        ...
        if not description:
            print('ERROR: Descripcion requerida ...{}'.format(name))
...
class Grader:
    ...
    def assertNewName(self, name):
        if name in [part.name for part in self.parts]:
            raise Exception("Parte del nombre %s que existe" % name)
    ...
    def gradePart(self, part):
        print('----- Parte Inicial %s%s: %s' % (part.name, ' (punto extra)' if part.extraCredit else '', part.description))
        ...
        print('----- Parte Final %s [toma %s (maximo permitido %s segundos), %s]' % (part.name, endTime - startTime, part.maxSeconds, displayPoints))
        print()
