...
if __name__ == '__main__':
	...
	print("\nPrimero, resolvemos G * y =  b:")
	y = np.linalg.solve(G, b)
	print("La solución de 'y' es:\n" +str(y.round(7)))

	print("\nLuego desarrollamos Gt * x = y:")
	x = np.linalg.solve(Gt, y)
	print("La solución de 'x' es: \n"+str(x.round(7)))

	R = x - np.linalg.solve(Ap, b)
	print("\nLa valor de R es: \n"+str(R.round(7)))
	print("Norma de R es: \n"+str(np.linalg.norm(R, np.inf).round(7)))
	print("Norma de b es: \n"+str(np.linalg.norm(b, np.inf).round(7)))
	print("Nro de condicion de 'Ap': \n"+str(np.linalg.cond(Ap, np.inf).round(7)))
